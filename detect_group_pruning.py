...
def run(
    ...
    DG.build_dependency(model.model, example_inputs=example_inputs)
    
    #intitial-layer
    for layer in model.model.model[:2]:
        if isinstance(layer, nn.Sequential):
            for sublayer in layer:
                if hasattr(sublayer, 'conv'):
                    pruning_idxs = list(range(sublayer.conv.out_channels // 2))  # prune 50% channels
                    group = DG.get_pruning_group(sublayer.conv, tp.prune_conv_out_channels, idxs=pruning_idxs)
                    if DG.check_pruning_group(group):
                        group.prune()

"""    for i in [2, 3, 4, 5, 6, 7,  8]:
        layer = model.model.model[i]
        for sublayer in [layer.cv1, layer.cv2, layer.cv3]:
            if hasattr(sublayer, 'conv'):
                pruning_idxs = list(range(sublayer.conv.out_channels // 2))  #50%
                group = DG.get_pruning_group(sublayer.conv, tp.prune_conv_out_channels, idxs=pruning_idxs)
                if DG.check_pruning_group(group):
                    group.prune()"""
    #mid-layers
    ...
    for i in [3, 5, 7]:
        ...
        if hasattr(layer, 'conv'):
            pruning_idxs = list(range(layer.conv.out_channels // 2)) 
            group = DG.get_pruning_group(layer.conv, tp.prune_conv_out_channels, idxs=pruning_idxs)
            if DG.check_pruning_group(group):
                group.prune()
