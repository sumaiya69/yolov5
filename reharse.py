...
def run(
    ...
    imp = tp.importance.MagnitudeImportance(p=2)  # Magnitude Importance
        ...
        for group in pruner.step(interactive=True):
            for dep, idxs in group:
                ...
                if pruning_fn in [tp.prune_conv_in_channels, tp.prune_linear_in_channels]:
                    ...
                    target_layer.weight.data[idxs] *= 0
                    target_layer.bias.data[idxs] *= 0
                #group.prune(idxs=[0, 2, 6])  # <= disable-hard-pruning

        for name, param in model.model.named_parameters():
            if "conv" in name and "weight" in name:
                print(f"After pruning step {i+1}: {name} -> {param}")
    ...
    pruned_model_path = "soft_pruned_model.pt"
    torch.save({
        'model': model.model,
